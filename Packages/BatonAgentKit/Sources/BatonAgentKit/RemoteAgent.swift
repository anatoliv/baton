...
public enum RemoteAgent {
    ...
    public typealias ToolRunner = @MainActor (RemoteToolCall) async -> (text: String, isError: Bool)
    /// Injection seam for tests: one model round trip.
    public typealias Turn = @MainActor ([RemoteAgentMessage], RemoteToolSchemas) async throws -> RemoteAgentStep
    ...
    public static func toolSchemas(definitions: [[String: Any]]) -> RemoteToolSchemas {
        let base = RemoteNaturalLanguage.toolSchemas(from: definitions)
...
public struct RemoteAgentMessage: Sendable {
...
public struct RemoteAgentStep: Sendable {
