...
public final class RemoteCommandRouter {
    ...
    /// actor — and everything it reaches (the injected tool surface)
    ...
    private func route(_ inbound: RemoteInbound) async -> RemoteReply? {
        ...
        case let .malformed(_, text, hint):
            ...
            // means the sentence was doing more than the verb's grammar allows
            // — "rate 4 this track and list similar by the same artist" is two
            // requests, and the parser can only see the first three characters
            // of the first one. Hand the whole thing to the model when there is
            // one; otherwise say what the verb wanted, not the entire manual.
            guard settings.naturalLanguage.isConfigured else { return .plain(hint) }
            switch await askTheModel(text, inbound) {
            case let .success(reply): return reply
            case .failure: return .plain(hint)
            }
    ...
    private func askTheModelOnce(
        ...
        do {
            let tools = RemoteNaturalLanguage.toolSchemas(from: self.tools.definitions())
    ...
    private func run(_ call: RemoteToolCall) async -> RemoteReply {
        let (text, isError) = await tools.run(
            name: call.name,
            arguments: call.jsonArguments,
            sessionID: nil
        )
...
public enum RemoteResultFormatter {
