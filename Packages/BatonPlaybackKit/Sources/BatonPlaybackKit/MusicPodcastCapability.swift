...
@MainActor
@Observable
public final class PodcastCapabilityStore {
    ...
    public enum Support: Equatable { case unknown, supported, unsupported }

    public private(set) var support: Support = .unknown
    ...
    public func probeIfNeeded() async {
    ...
    public func record(_ verdict: Support) {
    ...
    /// hiding.
    public static func classify(_ error: Error) -> Support? {
    ...
    public static let unsupportedTTL: TimeInterval = 7 * 24 * 60 * 60
    /// Injectable clock for tests.
    public nonisolated(unsafe) static var now: () -> Date = { Date() }

    public func persisted(for serverID: UUID) -> Bool? { // internal for  expiry test
