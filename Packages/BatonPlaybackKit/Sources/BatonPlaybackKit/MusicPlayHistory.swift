...
@MainActor
@Observable
public final class MusicPlayHistory: LocalListenRecording {
    ...
    public struct Entry: Codable, Identifiable, Hashable, Sendable {
        ...
        public init(song: NavidromeSong, playedAt: Date) {
            self.song = song
            self.playedAt = playedAt
        }
        public var id: String { "\(song.id)-\(playedAt.timeIntervalSince1970)" }
    }
    ...
    @ObservationIgnored public static let storageKey = "tonebox.music.playHistory"
    @ObservationIgnored public static let enabledKey = "tonebox.music.localLogEnabled"
    ...
    @ObservationIgnored public static let maxEntries = 200_000
    ...
    public func record(_ song: NavidromeSong) {
    ...
    public func record(_ song: NavidromeSong, playedAt: Date) {
    ...
    public var recentlyPlayed: [NavidromeSong] {
        var seen = Set<String>()
    ...
    public var lifetimeCount: Int { entries.count }

    /// When you first started listening (oldest entry), or nil if the archive is empty.
    public var firstListenDate: Date? { entries.last?.playedAt }

    /// The most recent listens as timestamped rows (the scrollable log).
    public func listenLog(limit: Int = 500) -> [Entry] { Array(entries.prefix(limit)) }

    /// Most-played tracks since `since`, as (song, count), highest first.
    public func topTracks(since: Date, limit: Int = 25) -> [(song: NavidromeSong, count: Int)] {
    ...
    public func topAlbums(since: Date, limit: Int = 25) -> [(album: String, count: Int, artwork: NavidromeSong)] {
    ...
    public func topArtists(since: Date, limit: Int = 25) -> [(artist: String, count: Int)] {
        rank(since: since, key: {
    ...
    public func playCount(since: Date) -> Int { entries.reduce(0) { $0 + ($1.playedAt >= since ? 1 : 0) } }
    ...
    public func lastPlayedByID() -> [String: Date] {
        var map: [String: Date] = [:]
    ...
    public func dailyCounts(since: Date, calendar: Calendar = .current, maxDays: Int = 400) -> [(day: Date, count: Int)] {
    ...
    public var portableListens: [PortableListen] {
    ...
    @discardableResult
    public func ingest(_ listens: [PortableListen]) -> Int {
