...
public struct Scrobble: Codable, Equatable, Sendable {
    ...
    public init(song: NavidromeSong, startedAt: Date) {
        songID = song.id
...
public struct QueuedScrobble: Codable, Equatable, Sendable, Identifiable {
    public let id: String
    public let destination: String
    public let scrobble: Scrobble
    public var attempts: Int

    public init(destination: String, scrobble: Scrobble, id: String = UUID().uuidString, attempts: Int = 0) {
        self.id = id
...
public final class ScrobbleQueue {
    public private(set) var pending: [QueuedScrobble] = []

    private let defaults: UserDefaults
    public static let storageKey = "tonebox.music.scrobbleQueue"
    ...
    public func enqueue(_ scrobble: Scrobble, destination: String) {
    ...
    /// Drops every queued listen without delivering it.
    ///
    /// Used when a session ends: this outbox holds the *previous* account's listens, and
    /// replaying them under whatever credentials come next would scrobble one person's
    /// music to another person's profile.
    public func clear() {
        pending.removeAll()
    ...
    public func take(destination: String, limit: Int) -> [QueuedScrobble] {
    ...
    public func resolve(_ delivered: [QueuedScrobble]) {
    ...
    public func fail(_ items: [QueuedScrobble], countsAsAttempt: Bool = true) {
    ...
    public var pendingDestinations: Set<String> { Set(pending.map(\.destination)) }
