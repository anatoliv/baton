...
@MainActor
@Observable
public final class MusicDownloadStore {
    public static let shared = MusicDownloadStore()
    ...
    public private(set) var failedDownloads: [String: NavidromeSong] = [:]
    /// Ids of failed downloads (compat shim over `failedDownloads`).
    public var failedIDs: Set<String> { Set(failedDownloads.keys) }
    ...
    public private(set) var downloadedAlbumCounts: [String: Int] = [:]
    ...
    public private(set) var downloadedCollections: [String: Set<String>] = [:]

    @ObservationIgnored public static let folderKey = "tonebox.music.downloadFolder"
    @ObservationIgnored public static let templateKey = "tonebox.music.downloadFilenameTemplate"
    /// Default filename template. Tokens: `{artist} {album} {title} {id}`.
    public static let defaultTemplate = "{artist} - {title}"
    public static let manifestName = ".tonebox-downloads.json"
    ...
    public static let metaName = ".tonebox-downloads-meta.json"
    ...
    public static let collectionsName = ".tonebox-downloads-collections.json"
    ...
    ///. Defaults to `.shared`.
    @ObservationIgnored public var urlSession: URLSession = .shared
    ...
    @ObservationIgnored public var lastPlayedProvider: (() -> [String: Date])?
    ...
    public func downloadedCount(albumID: String) -> Int { downloadedAlbumCounts[albumID] ?? 0 }

    public static func collectionKey(kind: String, id: String) -> String { "\(kind):\(id)" }
    ...
    public func registerCollection(kind: String, id: String, trackIDs: [String]) {
    ...
    public func collectionMemberCount(kind: String, id: String) -> (downloaded: Int, total: Int)? {
    ...
    public func filename(for song: NavidromeSong, ext: String = "mp3") -> String {
        Self.renderFilename(
    ...
    /// owns the same name.
    public nonisolated static func renderFilename(
        template: String,
    ...
    public func localURL(for songID: String) -> URL? {
    ...
    /// When installed (iOS), downloads run on a background URLSession that survives
    /// app suspension; enqueue returns immediately and the delegate finalizes.
    @ObservationIgnored public var backgroundEngine: BackgroundDownloadEngine?
    ...
    @discardableResult
    public func download(_ song: NavidromeSong) async -> Bool {
        guard !isDownloaded(song.id), !inFlight.contains(song.id) else { return isDownloaded(song.id) }
        if let backgroundEngine {
            inFlight.insert(song.id)
            downloadProgress[song.id] = 0
            backgroundEngine.enqueue(song)
            return true // enqueued; the delegate path finalizes (or records the failure)
        }
        inFlight.insert(song.id)
        downloadProgress[song.id] = 0
        defer {
        ...
        do {
            ...
            if let resume = persistedResumeData(for: song.id) {
                ...
                (temp, response) = try await urlSession.download(for: request, delegate: observer)
            }
            clearResumeData(for: song.id) // completed → the partial is no longer needed
            try finalizeDownload(of: song, temp: temp, response: response)
            return true
        } catch {
            downloadLog
    ...
    ///
    @discardableResult
    public func download(_ songs: [NavidromeSong]) async -> Int {
        var completed = 0
        for song in songs {
    ...
    /// Validation + commit for a completed download, shared by the in-process path and
    /// the background-session delegate path (which finishes downloads the async caller
    /// never sees — including after an app relaunch).
    ///
    /// : URLSession does NOT throw on 4xx/5xx; without validation a 404 or a
    /// reverse-proxy login page would be saved as a completed `.mp3` and then preferred
    /// over streaming forever. The MPL integrity checks also reject error-as-200
    /// envelopes and truncated transcodes, and the file is named from its magic bytes.
    func finalizeDownload(of song: NavidromeSong, temp: URL, response: URLResponse) throws {
        do {
        ...
        let declaredExt = Self.fileExtension(forContentType: (response as? HTTPURLResponse)?.value(forHTTPHeaderField: "Content-Type"))
        let ext = AudioContainer.sniff(atPath: temp.path)?.rawValue ?? declaredExt
    ...
    public static func validateDownloadResponse(_ response: URLResponse, fileURL: URL) throws {
        ...
        // MPL integrity checks (see MPL/AudioResponseValidator.swift): catches a Subsonic
        // error-as-200 XML/JSON envelope even when a proxy mangles the Content-Type, and a
        // close-delimited transcode that truncated mid-stream.
        do {
            try AudioResponseValidator.validate(fileAt: fileURL, response: response, songId: "", logger: downloadLog)
        } catch {
            throw DownloadError.notAudio(String(describing: error))
        }
    }
    ...
    /// Removes every download this store knows about — files, manifest and metadata.
    ///
    /// For session teardown: leaving a previous account's music on the device is the part
    /// of "disconnect" that most obviously isn't one. Goes through `delete(_:)` per id so
    /// the same provenance rule applies — a bare file Baton didn't write is never unlinked.
    public func deleteAll() {
        for songID in Array(manifest.keys) + Array(meta.keys) where downloadedIDs.contains(songID) || manifest[songID] != nil {
            delete(songID)
        }
        // Anything left in the maps had no file to remove; drop the bookkeeping too.
        manifest.removeAll()
        meta.removeAll()
    ...
    public func remove(id songID: String) { delete(songID) }
    ...
    public func downloadedItems() -> [DownloadItem] {
    ...
    public static func evictionPlan(items: [(id: String, bytes: Int64)], lastPlayed: [String: Date], capBytes: Int64) -> [String] {
    ...
    @discardableResult
    public func enforceStorageCap(lastPlayed: [String: Date]) -> [String] {
    ...
    public nonisolated static func fileByteSize(at url: URL) -> Int64 {
    ...
    public nonisolated static func parseFilename(_ base: String) -> (artist: String?, title: String) {
    ...
    public static func isPlausibleSubsonicID(_ s: String) -> Bool {
...
private final class DownloadProgressObserver: NSObject, URLSessionTaskDelegate {
    ...
    public init(songID: String, onProgress: @escaping @Sendable (String, Double) -> Void) {
        self.songID = songID
...
extension MusicDownloadStore {
    /// Delegate entry: validation + commit for a background-completed download.
    public func completeBackgroundDownload(song: NavidromeSong, temp: URL, response: URLResponse) {
        inFlight.remove(song.id)
        downloadProgress[song.id] = nil
        clearResumeData(for: song.id)
        do {
            try finalizeDownload(of: song, temp: temp, response: response)
        } catch {
            failedDownloads[song.id] = song
    ...
    public func noteBackgroundProgress(id: String, fraction: Double) {
        guard inFlight.contains(id) else { return }
        downloadProgress[id] = fraction
    }

    public func noteBackgroundFailure(_ song: NavidromeSong, resumeData: Data? = nil) {
        inFlight.remove(song.id)
        downloadProgress[song.id] = nil
        if let resumeData { saveResumeData(resumeData, for: song.id) }
        failedDownloads[song.id] = song
    }

    /// A task found alive after relaunch — resurrect its in-flight UI state.
    public func noteBackgroundRestored(_ song: NavidromeSong) {
        guard !isDownloaded(song.id) else { return }
        inFlight.insert(song.id)
        if downloadProgress[song.id] == nil { downloadProgress[song.id] = 0 }
    }
}
