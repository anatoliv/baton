...
@MainActor
public final class GaplessPrefetcher {
    private var tasks: [String: Task<Void, Never>] = [:]
    private let cache: MusicGaplessCache
    private let downloader: @MainActor (URL, String) async -> URL?

    public init(cache: MusicGaplessCache, downloader: @escaping @MainActor (URL, String) async -> URL?) {
        self.cache = cache
    ...
    public func cachedURL(for songID: String) -> URL? { cache.localURL(for: songID) }

    /// Whether a prefetch is already in flight for this song (so the caller doesn't start a second).
    public func isPrefetching(_ songID: String) -> Bool { tasks[songID] != nil }

    public var cacheSizeBytes: Int64 { cache.sizeBytes() }
    ...
    public func reap(keeping plannedID: String?) {
