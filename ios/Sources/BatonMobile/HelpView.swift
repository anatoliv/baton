    ...
    static let releases: [ReleaseNote] = [
        ReleaseNote(
            ...
            changes: [
                ...
                .init(.fixed, "A locked keychain used to look exactly like having no password saved, so a phone that was already set up landed back on the setup screen with no explanation. It now says the keychain is locked, and that typing your password in again will not fix it. Unlock your phone and open the screen again."),
                .init(.fixed, "The Lyrics sheet now tells you when the lookup failed rather than saying the track has no lyrics, and missing album artwork prompts Baton to re-check your sign-in instead of leaving a blank grid."),
                .init(.fixed, "The music friend's microphone turns off when you switch tabs mid-recording, and a permission you granted after a refusal is picked up the next time you open the friend."),
                .init(.fixed, "A crash while Baton was saving its list of downloaded tracks no longer makes the downloaded library read as empty. The files are found again from the notes Baton keeps alongside them."),
                .init(.improved, "Transcribing a long podcast no longer holds the whole episode in memory while it uploads, and the equalizer no longer risks a glitch in the sound while you drag a slider."),
