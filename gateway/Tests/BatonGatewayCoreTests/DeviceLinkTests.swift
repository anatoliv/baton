...
final class DeviceLinkTests: XCTestCase {
    ...
    /// A command arriving with nobody parked queues, and the next poll takes it. Unchanged
    /// behaviour, asserted because the fix rewrote the branch either side of it.
    func testACommandWithNoDeviceWaitingIsQueuedForTheNextPoll() async {
        ...
        let queued = await withDeadline { () -> DeviceLink.Command? in
            ...
            return await link.awaitCommand(timeout: 1.0)
        }
