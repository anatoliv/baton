...
final class WebhookActionsTests: XCTestCase {
    ...
    func testFormBodyIsURLEncoded() throws {
        var action = WebhookAction(name: "Save", urlTemplate: "https://web-host/save")
    ...
    func testURLWithSpacesInTokenStaysValid() throws {
        let action = WebhookAction(name: "S", urlTemplate: "https://web-host/save?t={title}")
        let req = try XCTUnwrap(WebhookTemplate.buildRequest(action, tokens: ["title": "Hello World"]))
        XCTAssertEqual(req.url?.absoluteString, "https://web-host/save?t=Hello%20World")
    }
    ...
    func testBuildRequestPostJSON() throws {
        var action = WebhookAction(name: "Save", urlTemplate: "https://web-host/save?u={enclosureUrl}")
        ...
        XCTAssertEqual(req.url?.absoluteString, "https://web-host/save?u=https%3A%2F%2Fcdn%2Fep.mp3")
    ...
    func testGetHasNoBody() throws {
        var action = WebhookAction(name: "Ping", urlTemplate: "https://web-host/ping")
        action.method = .get
    ...
    func testCredentialedTokensStrippedWhenToggleOff() async {
        ...
        var action = WebhookAction(name: "Save", urlTemplate: "https://web-host/save?u={streamUrl}")
    ...
    func testRunReportsSuccessAndFailure() async {
        ...
        let store = WebhookActionStore(defaults: freshDefaults(), send: { req in captured = req; return 204 },
                                       sendWithBody: nil)
        let action = WebhookAction(name: "Save", urlTemplate: "https://web-host/save")
        let ok = await store.run(action, tokens: [:])
        XCTAssertTrue(ok.ok)
        XCTAssertEqual(captured?.url?.absoluteString, "https://web-host/save")
